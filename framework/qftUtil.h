 ...
 */
inline TLorentzVector qfttoROOT(const Vector4<double> &p4qft) {
  const TLorentzVector p4ROOT(p4qft.Px(), p4qft.Py(), p4qft.Pz(), p4qft.E());

  return p4ROOT;
}

inline Vector4<double> ROOTtoqft(const TLorentzVector &pROOT) {
 ...
 */
inline Vector4<double> Minimaltoqft(const Minimal4Vector &p4Min) {
