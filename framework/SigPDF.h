  ...
  std::complex<double> GetAmp(const Event &event,
    ...
    const auto sum_amps =
            [&amp, &_event = std::as_const(event), &_par = std::as_const(par),
                    &CP_conf = std::as_const(CP_conf)](const auto &...Amps) {
              Eigen::Index i = 0;
              ((amp +=
                        Amps.GetCoupling(_par, CP_conf) *
                        _event.GetAmps(CP_conf)(i++)),
                      ...);
            };
...
#pragma omp parallel for reduction(                                            \
    ...
    for (unsigned int i = 0; i < nEvents; ++i) {
      ...
      std::apply(
              [&_event = event, &_par = std::as_const(par)](
                      const auto &...Amps) {
                std::size_t j = 0;
                const auto lambda_amp = [&_event = _event,
                  ...
                  const std::complex<double> a =
                          sqrt(_event.GetEff()) *
                          Amp.GetAmp(_event, _par, CPConf::A);
                  const std::complex<double> abar =
                          sqrt(_event.GetEff()) *
                          Amp.GetAmp(_event, _par, CPConf::Abar);
