...
public:
  ...
  double GetPDF(const Event &event, const double &time, const int &qtag,
    ...
    const auto q = static_cast<double>(qtag);
  ...
  std::complex<double> GetAmp(const Event &event,
    ...
    const auto sum_amps = [&amp, &_event = std::as_const(event),
                           &_par = std::as_const(par),
                           &CP_conf =
                               std::as_const(CP_conf)](const auto &...Amps) {
      Eigen::Index i = 0;
      ((amp += Amps.GetCoupling(_par, CP_conf) * _event.GetAmps(CP_conf)(i++)),
       ...);
    };
...
#pragma omp parallel for reduction(                                            \
  ...
  inline void ComputePhases(const std::vector<double> &par) const {
    std::fill(_phases, _phases + _nAmps, 0.0);

    // auto calculate_phase = [&](auto &&...amps) {
    auto calculate_phase = [this, &_par = std::as_const(par)](auto &&...amps) {
      size_t i = 0;
      (
          [&](const auto &amp) {
            _phases[i] = std::arg(amp.GetCoupling(_par, CPConf::A));
            ++i;
          }(amps),
          ...); // Fold expression over tuple elements
    };

    std::apply(calculate_phase, _Amps);
  }

  const double &GetPhase(const unsigned int &idx_amp) const {
    return _phases[idx_amp];
  }
