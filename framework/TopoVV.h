...
public:
    [[nodiscard]] double GetAmpBarrier(const unsigned int &L_conf,
    ...
    AmpsBarrier(const std::array<Minimal4Vector, nBody> &p4s) {
    ...
    [[nodiscard]] std::complex<double> GetAmpSpin(const unsigned int &L_conf,
                                                  const unsigned int &CP_conf) const {
        switch (CP_conf) {
            case CPConf::A:
    ...
    void
    SetAmpsSpin(const AmpSpinMap &amp_spin, const AmpSpinMap &amp_spin_CP) {
        _amp_spin = amp_spin;
