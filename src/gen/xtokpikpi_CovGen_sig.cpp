...
int main(const int argc, const char *argv[]) {
  ...
  op("hmax,h", po::value<double>(&hmax), "Maximum height of the distribution");
  ...
  for (unsigned int i = 0; i < nevents; ++i) {
    ...
    while (true) {
      ...
      const double tauLong =
          1.0 / ((1.0 / mn_param.Value(mother.getTauIdx())) -
                 fabs(mn_param.Value(mother.getDGIdx()) / 2.0));
